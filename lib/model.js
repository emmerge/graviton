...
Model.prototype.inc = function(thing, increment) {
  if (_.isObject(thing)) {
    console.error('Model.prototype.inc does not yet support object parameter');
  }
  var val = this.get(thing);
  this._setProperty(thing, val + increment);
  var obj = {};
  obj[thing] = increment;
  var $inc = {$inc: {}};
  $inc.$inc[thing] = increment;
...
Model.prototype._pushProperty = function(key, value) {
  ...
  if (!_.isArray(array))
    throw new Error("Trying to push a value onto a non-array");
  if (_.isArray(value)) {
