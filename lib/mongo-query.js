...
function _getObj(keyOrObj, value) {
...
MongoModifier = class MongoModifier {
  ...
  mergeUpdate(mongoUpdate) {
    for (let type of updateOperators) {
      if (_.isObject(mongoUpdate[type])) {
        let obj = mongoUpdate[type];
        if (type === '$set') {
          obj = MongoModifier.flattenObject(obj);
        }
  ...
  unset(keyOrObj) {
    var obj = _getObj(keyOrObj, "");
    _mongoExtend(this.$unset, obj);
    return this;
  }

  addToSet(keyOrObj, value) {
    var obj = _getObj(keyOrObj, value);
...
MongoQuery = class MongoQuery extends MongoModifier {
  ...
  and(mongoQuery) {
    ...
    if (_keysHaveDollars(mongoQuery._selector) || _keysHaveDollars(this._selector)) {
      selector = {$and: [this.selector, mongoQuery.selector]};
    } else {
      selector = _.extend({}, this.selector, mongoQuery.selector);
    }
