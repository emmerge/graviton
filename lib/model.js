Model = class Model {
  constructor(collection, obj) {
    ...
    this.attributes = _.extend(obj, this.constructor._defaults);

    for (let rel of this.constructor._relations) {
      this[rel.relationName] = rel.generate(this);
    }
  ...
  static relations(relationsObj) {
    ...
    for (let relationType of Object.keys(relationsObj)) {
      for (let relationName of Object.keys(relationsObj[relationType])) {
        if (_.findWhere(relations, {relationName: relationName})) {
   ...
   */
  static defaults(defaultsObj) {
    this._defaults = _deepClone(defaultsObj);
    return this;
  }

  static mixin(modelClass) {
    _.defaults(this.prototype, modelClass.prototype);
    for (let rel of modelClass._relations) {
      let existing = _.findWhere(this._relations, {relationName: rel.relationName});
      if (!existing) {
