...
Model.prototype.remove = function(callback) {
  ...
  if (this._options.relations) {
    if (this._options.relations.hasMany || this._options.relations.hasOne) {
      relations = [];
      if (this._options.relations.hasMany)
        relations = relations.concat(_.pairs(this._options.relations.hasMany));
      if (this._options.relations.hasOne)
        relations = relations.concat(_.pairs(this._options.relations.hasOne));
      //console.log('has[One|Many] relations', relations);
    }
  }

  // before destroying the parent, check for relational restrictions
  _.each(relations, function(relation){
    var relationName = relation[0];
    var relationOptions = relation[1];

    if (relationOptions.onRemove == 'deny') {
      //console.log('collection:', self._collection._name, 'relation:',  relationName, 'should restrict');
      if (self[relationName].findOne())
        throw Error('Cannot remove record, relation \''+relationName+'\' denies removal if child records exist.');
    }
  });
  ...
  _.each(relations, function(relation) {
    ...
    if (relationOptions.onRemove == 'cascade') {
      //console.log('collection:', self._collection._name, 'relation:',  relationName, 'should onRemove cascade');
      _.each(self[relationName].all(), function(child){
        console.log('child', relationOptions.collection, 'to delete:', child._id);
        child.remove();
      })
    }

    if (relationOptions.onRemove == 'unset') {
      //console.log('collection:', self._collection._name, 'relation:',  relationName, 'should remove reference');
      _.each(self[relationName].all(), function(child){
        console.log('child', relationOptions.collection, 'to remove reference to:', child._id);
        var updateObj = {};
        updateObj['$unset'] = {};
        updateObj['$unset'][relationOptions.foreignKey] = true;
