function BelongsTo(model, config) {
  return function() {
    return Graviton.getCollection(config).findOne({
      _id: model.get(config.field)
    });
  };
}

function HasOne(model, config) {
  return function() {
    return Graviton.getCollection(config).findOne({
      [config.foreignKey]: model._id
    });
  };
}

class Relation extends MongoQuery {
  ...
  static get(type) {
    return relationConstructors[type];
  }
}
this.Relation = Relation;

class HasMany extends Relation {
  constuctor(model, config) {
...
const relationConstructors = {
  belongsTo: BelongsTo,
  hasOne: HasOne,
  hasMany: (mdl, cfg) => new HasMany(mdl, cfg),
  belongsToMany: (mdl, cfg) => new BelongsToMany(mdl, cfg)
};
