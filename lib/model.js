Model = class Model {
  constructor(collection, obj) {
    ...
    for (let rel of this.constructor._relations) {
      let cstr = Relation.get(rel.type);
      this[rel.relationName] = cstr(this, rel);
    }

    this._saveQuery = new MongoQuery(collection);
  }
  /**
   * Completely re-define the relations for this class
   * Pass an empty object to clear all inherited relations
   */
  static relations(relationsObj) {
    var relations = [];
    for (let relationType of Object.keys(relationsObj)) {
      for (let relationName of Object.keys(relationsObj[relationType])) {
        ...
        let relation = Relation.get(relationType);
        if (relation) {
  ...
  save(callback) {
    if (this._id) {
      ...
      return _performSaveOperation.call(this, 'update', callback);
    } else {
      return _performSaveOperation.call(this, 'insert', callback);
    }
  }
};
Model._relations = [];
...
function _performSaveOperation(method, callback) {
  if (callback) {
    this._saveQuery[method]((err) => {
      if (!err) {
    ...
    var result = this._saveQuery[method]();
    this._saveQuery.reset();
    return result;
  }
}
