...
hasOne = function(config) {
  return function(model) {
    ...
    var field = config.field || '_id';
    if (model) {
      model = collection.build(model);
      if (model.get(config.foreignKey) !== this.get(field)) {
        model.set(config.foreignKey, this.get(field)).save();
      } 
      return model;
    } else {
      var criteria = {};
      criteria[config.foreignKey] = this.get(field);
      console.log('criteria', criteria, field);
