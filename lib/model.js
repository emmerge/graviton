  ...
  if (callback) {
    ...
    if (!this.persist()) {
      for (var i in this._pendingMods) {
        // @see https://trello.com/c/QHW5FBf1/383-graviton-model-save-frequently-triggers-a-mongodb-driver-bug-invalid-modifier-modifier-must-be-an-object
        try {
          this._collection.update(this._id, this._pendingMods[i]);
        }
        catch (x) {
          if (x.message && x.message.match(/Invalid modifier. Modifier must be an object./)) {
            console.log("Warning: caught Invalid modifier error!", "collection:", _collection._name, "id:", this._id, "updates:", this._pendingMods[i]);
            // Try again, once. Don't catch it. If the bug is transient then it will succeed and log such.
            this._collection.update(this._id, this._pendingMods[i]);
            console.log("Notice: the Invalid modifier error is transient");
          }
          else
            throw x;
        }
