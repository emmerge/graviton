...
class Relation extends MongoQuery {
  constructor(collection, model, config) {
    super(collection);
    this.model = model;
    this.config = config;
  }
...
class HasMany extends Relation {
  constructor(model, config) {
    super(Graviton.getCollection(config), model, config);
    console.log('creating hasmany with', model, config);
    this.selector = {[config.foreignKey]: model._id};
  }
}
...
const relationConstructors = {
  belongsTo: BelongsTo,
  hasOne: HasOne,
  hasMany: function(mdl) {
    return new HasMany(mdl, this);
  },
  belongsToMany: function(mdl) {
